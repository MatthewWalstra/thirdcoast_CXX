...
namespace g3 {
      ...
      , _firstEntry(true)
   {
      ...
      if (!isValidFilename(_log_prefix_backup)) {
         std::cerr << "thirdcoast/g3log/g3log: forced abort due to illegal log prefix [" << log_prefix << "]" << std::endl;
         abort();
      }
   ...
   FileSink::~FileSink() {
      std::string exit_msg {"thirdcoast/g3log/g3log g3FileSink shutdown at: "};
